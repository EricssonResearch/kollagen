...
Data get_data(const DataGenerator &data_generator) {
...
int main() {
  ...
  auto agent_param =
      AgentParams(1.0, 4, sigma_pos, sigma_ang, Idiag, Idiag * 4, 1406, false);
  auto agent2_param =
      AgentParams(1.0, 4, sigma_pos, sigma_ang, Idiag, Idiag * 4, 1302, false);
  auto agent3_param =
      AgentParams(1.0, 4, sigma_pos, sigma_ang, Idiag, Idiag * 4, 3096, false);
  auto agent4_param =
      AgentParams(1.0, 4, sigma_pos, sigma_ang, Idiag, Idiag * 4, 7324, false);
  auto agent5_param =
      AgentParams(1.0, 4, sigma_pos, sigma_ang, Idiag, Idiag * 4, 2443, false);

  auto inter_lc_param =
      InterLCParams(sigma_pos_inter, sigma_ang_inter, Idiag_inter,
                    Idiag_inter, radius, 1.0, prob_scale, 2708);

  DataGenerator data_generator = DataGenerator(
      {{agent_param, agent2_param, agent3_param, agent4_param, agent5_param},
       lc_param,
       inter_lc_param,
       N_steps});
  ...
  for (int i = 1; i < number_of_steps; ++i) {
    ...
    if (all_connected) {
      ...
      /* for (int j = 0; j < number_of_agents; ++j) { */
      /*   if (i==1) { */
      /*     auto KeyPosePair = gt_map.at(0).at(j); */
      /*     online_result.insert(KeyPosePair.first, KeyPosePair.second); */
      /*   } */
      /*   auto KeyPosePair = gt_map.at(i).at(j); */
      /*   online_result.insert(KeyPosePair.first, KeyPosePair.second); */
      /* } */
  ...
  auto all_agents_TUM = open_file("iSAM2_all.tum");;
  int offset = 0;
  ...
  for (int i = 0; auto const &agent : data_generator.Walkers()) {
    ...
    while (true) {
      try {
        ...
        write_TUM_vertices(all_agents_TUM, Values_to_XYTheta(temp_result), offset);
        offset += static_cast<int>(temp_result.size());

        break;
      }
    }
    save_binary("APE_agent" + std::to_string(agent.ID()) + ".bin", APE.at(i));
    i++;
  }

  save_binary("APE_time.bin", timesteps);
